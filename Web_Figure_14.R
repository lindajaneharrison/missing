# Web Figure 14 (Sample Size Calculation for Randomized Clinical Trials via Inverse Probability of Response Weighting 
...
kappa <- 0.5
for (m in c(2,5,10)){
  n_stnadard_out <- n_standard(power=0.9,alpha=0.05,kappa=0.5,mu_1=0.475,mu_0=0.375,
...
fig_n <- ggplot() + 
  theme_classic() +
  geom_point(data=combine,aes(x=m,y=n_standard,color="iv. C-standard",shape="iv. C-standard")) + 
  geom_point(data=combine,aes(x=m,y=n_known,color="ii. C-known",shape="ii. C-known")) + 
  geom_point(data=combine,aes(x=m,y=n_approx,color="iii. C-approx",shape="iii. C-approx")) +
  geom_point(data=combine,aes(x=m,y=n_IPRW,color="i. C-IPRW",shape="i. C-IPRW")) + 
  ...
  labs(x = TeX("$m$"),y="n",color="") + theme(plot.title = element_text(size=10))
fig_n 
fig_power <- ggplot() + 
  theme_classic() +
  geom_point(data=combine,aes(x=m,y=empirical.power..standard.eqn.4.,color="iv. C-standard",shape="iv. C-standard")) + 
  geom_point(data=combine,aes(x=m,y=empirical.power..known.eqn.4.,color="ii. C-known",shape="ii. C-known")) +
  geom_point(data=combine,aes(x=m,y=empirical.power..approx.eqn.4.,color="iii. C-approx",shape="iii. C-approx")) +
  geom_point(data=combine,aes(x=m,y=empirical.power..IPRW.eqn.4.,color="i. C-IPRW",shape="i. C-IPRW")) +
  scale_y_continuous(breaks=c(75,80,85,90,95,100), lim=c(74,100)) +
  scale_x_continuous(breaks=c(2,5,10)) +
  scale_colour_manual(values=cbPalette) + scale_shape_manual(values=nice.shapes) +
  guides(color=guide_legend(override.aes = list(color=cbPalette, shape=nice.shapes)), shape=FALSE) +
  labs(x = TeX("$m$"),y="power",color="") + theme(plot.title = element_text(size=10))
fig_power
...
ggsave("Fig_CRT_cont_n_m.pdf",scale=0.5)
...
aplot::plot_list(out_plot,legend,ncol=1,heights=c(3,0.1))
ggsave("Fig_CRT_cont_power_m.pdf",scale=0.5)
