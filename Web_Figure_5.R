# Web Figure 5 (Sample Size Calculation for Randomized Clinical Trials via Inverse Probability of Response Weighting when Outcome Data are Missing at Random)
...
mu_x <- 0
kappa <- 0.5
for (phi in c(0.75,0.775,0.8)){
  f1 <- function(beta_11) sum(plogis(1.4+(sqrt(2)*sqrt(sigma_x_sq)*x_j+mu_x)*beta_11)*w_j)/sqrt(pi)-phi
  f0 <- function(beta_10) sum(plogis(2+(sqrt(2)*sqrt(sigma_x_sq)*x_j+mu_x)*beta_10)*w_j)/sqrt(pi)-phi
  beta_11 <- uniroot(f1, c(0,3))$root
  beta_10 <- uniroot(f0, c(0,3))$root
  print(beta_11)
  print(beta_10)
  n_standard_out <- n_standard(power=0.9,alpha=0.05,kappa=0.5,mu_1=0.475,mu_0=0.375,
           type="continuous",link="identity",sigma_y_sq=0.245,phi=phi)
...
fig_n <- ggplot() + 
  theme_classic() +
  geom_point(data=combine,aes(x=phi,y=n_standard,color="iv. standard",shape="iv. standard")) + 
  geom_point(data=combine,aes(x=phi,y=n_known,color="ii. known",shape="ii. known")) + 
  geom_point(data=combine,aes(x=phi,y=n_approx,color="iii. approx",shape="iii. approx")) +
  geom_point(data=combine,aes(x=phi,y=n_IPRW,color="i. IPRW",shape="i. IPRW")) + 
  scale_y_continuous(breaks=c(2000,4000,6000,8000),lim=c(1000,8000)) +
  ...
  labs(x = TeX("$\\phi$"),y="n",color="") + theme(plot.title = element_text(size=10))
fig_n 
fig_power <- ggplot() + 
  theme_classic() +
  geom_point(data=combine,aes(x=phi,y=empirical.power..standard.eqn.4.,color="iv. standard",shape="iv. standard")) + 
  geom_point(data=combine,aes(x=phi,y=empirical.power..known.eqn.4.,color="ii. known",shape="ii. known")) +
  geom_point(data=combine,aes(x=phi,y=empirical.power..approx.eqn.4.,color="iii. approx",shape="iii. approx")) +
  geom_point(data=combine,aes(x=phi,y=empirical.power..IPRW.eqn.4.,color="i. IPRW",shape="i. IPRW")) +
  scale_y_continuous(breaks=c(75,80,85,90,95,100), lim=c(74,100)) +
  scale_x_continuous(breaks=c(0.75,0.775,0.8)) +
  scale_colour_manual(values=cbPalette) + scale_shape_manual(values=nice.shapes) +
  guides(color=guide_legend(override.aes = list(color=cbPalette, shape=nice.shapes)), shape=FALSE) +
  labs(x = TeX("$\\phi$"),y="power",color="") + theme(plot.title = element_text(size=10))
fig_power
...
ggsave("Fig_IRT_cont_n_phi.pdf",scale=0.5)
...
aplot::plot_list(out_plot,legend,ncol=1,heights=c(3,0.1))
ggsave("Fig_IRT_cont_power_phi.pdf",scale=0.5)
