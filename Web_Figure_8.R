...
mu_x <- 0
kappa <- 0.5
for (beta_11 in c(0.21,1.21,2.21)){
  f1 <- function(beta_01) sum(plogis(beta_01+(sqrt(2)*sqrt(sigma_x_sq)*x_j+mu_x)*beta_11)*w_j)/sqrt(pi)-0.8
  beta_01 <- uniroot(f1, c(0,100))$root
  print(beta_11)
  ...
  n_IPRW <- n_IPRW_cont(power=0.9,alpha=0.05,kappa=0.5,mu_1=0.475,mu_0=0.375,
                    ...
                    beta_01=beta_01,beta_11=beta_11,beta_00=2,beta_10=1.64)
  ...
  names(n_out) <- c("beta_11","n_standard","n_IPRW","n_known","n_approx")
...
fig_n <- ggplot() + 
  theme_classic() +
  geom_point(data=combine,aes(x=beta_11,y=n_standard,color="iv. standard",shape="iv. standard")) + 
  geom_point(data=combine,aes(x=beta_11,y=n_known,color="ii. known",shape="ii. known")) + 
  geom_point(data=combine,aes(x=beta_11,y=n_approx,color="iii. approx",shape="iii. approx")) +
  geom_point(data=combine,aes(x=beta_11,y=n_IPRW,color="i. IPRW",shape="i. IPRW")) + 
  scale_y_continuous(breaks=c(1000,2000,3000,4000),lim=c(1000,4000)) +
  scale_x_continuous(breaks=c(0.21,1.21,2.21)) +
  ...
  labs(x = TeX("$\\beta_{11}$"),y="n",color="") + theme(plot.title = element_text(size=10))
fig_n 
fig_power <- ggplot() + 
  theme_classic() +
  geom_point(data=combine,aes(x=beta_11,y=empirical.power..standard.eqn.4.,color="iv. standard",shape="iv. standard")) + 
  geom_point(data=combine,aes(x=beta_11,y=empirical.power..known.eqn.4.,color="ii. known",shape="ii. known")) +
  geom_point(data=combine,aes(x=beta_11,y=empirical.power..approx.eqn.4.,color="iii. approx",shape="iii. approx")) +
  geom_point(data=combine,aes(x=beta_11,y=empirical.power..IPRW.eqn.4.,color="i. IPRW",shape="i. IPRW")) +
  scale_y_continuous(breaks=c(75,80,85,90,95,100), lim=c(74,100)) +
  scale_x_continuous(breaks=c(0.21,1.21,2.21)) +
  scale_colour_manual(values=cbPalette) + scale_shape_manual(values=nice.shapes) +
  guides(color=guide_legend(override.aes = list(color=cbPalette, shape=nice.shapes)), shape=FALSE) +
  labs(x = TeX("$\\beta_{11}$"),y="power",color="") + theme(plot.title = element_text(size=10))
fig_power
...
ggsave("Fig_IRT_cont_n_inform.pdf",scale=0.5)
...
aplot::plot_list(out_plot,legend,ncol=1,heights=c(3,0.1))
ggsave("Fig_IRT_cont_power_inform.pdf",scale=0.5)
